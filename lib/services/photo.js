...
export class PhotoService {
  ...
  async get ({ id, type }) {
    try {
      this.log.info({ id, type }, 'Get Photos By Ids: Start')

      if (type === 'event') return this.getByEvent({ eventId: id })
      if (type === 'team') return this.getByTeam({ teamId: id })
      if (type === 'player') return this.getByPlayer({ playerId: id })
  ...
  async getByEvent ({ eventId }) {
    try {
      this.log.info({ eventId }, 'Get Photos By Event: Start')
      ...
      this.log.error({ err }, 'Get Photos By Event: Fail')
      throw err
    }
  }
