...
export class UserService {
  ...
  async email ({ email, amount, photoIds }) {
    try {
      ...
      const urls = await photoIds.map(id => {
        return this.s3Service.createDownloadUrl({ imageId: id })
      })

      // console.log('url2', Promise.all(Promise.all(urls)))
      //
      // create reusable transporter object using the default SMTP transport
      var transporter = nodemailer.createTransport({
       service: 'gmail',
       auth: {
        user: 'hmhouston7@gmail.com',
        pass: '*Dallas7'
          }
      })
      ...
      let info = await transporter.sendMail({
        ...
        text: `Hello ${urls} world?`,
        html: '<b>Hello world?</b>'
      })

      console.log('info: ', info)

      this.log.info({ email }, 'Email User: Success')
